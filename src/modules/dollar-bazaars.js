  ...
  SLINK.modules.register({
    ...
    async start(context) {
      ...
      function render(status) {
        current = status;
        setStatus();
        const items = Array.isArray(status?.items) ? [...status.items].sort((left, right) => Number(right.totalValue) - Number(left.totalValue)) : [];
        const next = Number(status?.nextRefreshAt) ? new Date(status.nextRefreshAt).toLocaleTimeString([], { hour:'numeric', minute:'2-digit' }) : '—';
        ui.getContentElement().innerHTML = `<div class="slink-dollar-summary"><div><strong>${items.length}</strong><span>listings</span></div><div><strong>${status?.fetchedAt ? escapeHtml(relativeTime(status.fetchedAt)) : '—'}</strong><span>updated</span></div><div><strong>${escapeHtml(next)}</strong><span>next API update</span></div></div>
          <div class="slink-dollar-list">${items.length ? items.map(item => `<article class="slink-dollar-row"><header><div><strong>${escapeHtml(item.itemName)} [${Number(item.itemId)}]</strong><small>${escapeHtml(item.sellerName)} [${Number(item.sellerId)}] · ${Number(item.quantity).toLocaleString('en-US')} available${item.itemType ? ` · ${escapeHtml(item.itemType)}` : ''}</small></div><div class="slink-dollar-total"><strong>${money(item.totalValue)}</strong><small>${money(item.marketPrice)} each</small></div></header><footer><small>${item.updatedAt ? `Checked ${escapeHtml(relativeTime(item.updatedAt))}` : ''}</small><a href="${escapeHtml(item.href)}" target="_blank" rel="noopener noreferrer">Open bazaar</a></footer></article>`).join('') : `<div class="slink-dollar-empty">${status?.lastError ? 'The refresh failed and there are no saved results yet.' : 'Weaver returned no active $1 Bazaar listings.'}</div>`}</div>`;
      }
