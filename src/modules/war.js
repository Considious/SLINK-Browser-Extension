  ...
  SLINK.modules.register({
    ...
    async start(context) {
      ...
      function targetCards() {
        ...
        return members.map(member => {
          ...
          return `<article class="slink-war-card">
            ...
            <div class="slink-war-meta"><span class="slink-war-pill ${member.activity === 'Online' ? 'slink-war-online' : ''}">${escape(member.activity)}</span><span class="slink-war-pill ${hospitalized ? 'slink-war-hospital' : ''}">${escape(member.statusState || 'Okay')}${hospitalized ? ` ${duration(remaining)}` : ''}</span><span class="slink-war-pill">FF ${Number.isFinite(member.fairFight) ? member.fairFight.toFixed(2) : '?'}</span>${member.lastActionRelative ? `<span>${escape(member.lastActionRelative)}</span>` : ''}</div>
      ...
      function render() {
        ...
        const tabs = ['targets', ...(canViewLogs ? ['logs'] : []), 'settings'];
        const body = activeTab === 'targets' ? targetCards() : activeTab === 'logs' ? logCards() : settingsHtml();
