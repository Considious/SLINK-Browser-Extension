  ...
  async function refresh() {
    ...
    try {
      ...
      elements.war.textContent = status.war.session.authenticated
        ? (status.war.activeWar?.opponentName || 'Ready / checking Torn API')
      ...
      const admin = SLINK.core.permissions.hasScope(status.permissions, 'admin.*');
      document.getElementById('diagnostic-section').hidden = !admin;
      elements.runDiagnostic.hidden = !admin;
      elements.lastDiagnostic.closest('div').hidden = !admin;
      ...
      if (admin) renderDiagnostic(status.lastDiagnostic);
    } catch (error) {
